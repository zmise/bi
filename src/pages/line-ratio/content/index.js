...
module.exports = {
  ...
  listen: {
    ...
    resetForm: function () {
      ...
      this.time.update({
        maxDate: new Date(targetDate.getFullYear(), targetDate.getMonth(), 1),
        minDate: new Date(2016, 0, 1)
      });
      targetDate.setMonth(targetDate.getMonth() - 2);
      this.time.selectDate(targetDate);

      // this.status.clearValue();
    ...
    queryParams: function () {
      ...
      // 其他参数
      p.time = this.time.el.value + '-01';
      // p.status = this.status.value.id;
    ...
    renderStat: function () {
      var _this = this;
      $.ajax({
        url: '/bi/marketing/orgCallIn/stat.json',
        data: {
        ...
        _this.$('#staticsis').html(summaryTpl(res.data));
      });
    },
    // 列表渲染
      ...
      this.list = $('#list').table({
        ...
        cols: [{
          ...
          renderer: function (val, item, rowIndex) {
            switch (val) {
              case 'RED': return '<span class="red-desc">表现较差</span>';
              case 'YELLOW': return '<span class="yellow-desc">需要加强</span>';
              case 'GREEN': return '<span class="green-desc">表现优异</span>';
            }
          }
        }],
        method: 'get',
        url: '/bi/marketing/orgCallIn/subStat.json',
        params: function () {
          return {
            statMonth: _this.params.time,
